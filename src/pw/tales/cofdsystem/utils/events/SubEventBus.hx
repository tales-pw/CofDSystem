...
class SubEventBus implements IEventBus {
    ...
    public function new(parent:IEventBus, filter:EventFilter = null) {
        ...
        if (filter == null) filter = SubEventBus.NO_FILTER;
        this.filter = filter;
    }
    ...
    public function addHandler<T: IEvent>(type:Class<T>, handler:(T) -> Void, priority:Int = null):EventHandlerRecord<T> {
        ...
        var record = this.parent.addHandler(type, (event:T) -> {
            if (this.filter(event)) handler(event);
        },
        priority);
    ...
    public function enable() {
        this.enabled = true;
        for (record in handlers) this.parent.addHandlerRecord(record);
    }

    public function disable() {
        this.enabled = false;
        for (record in handlers) this.parent.removeHandlerRecord(record);
    }

    private function checkEnabled() {
        if (!this.enabled) throw "Attempt to use disabled sub bus.";
    }
}
