...
class WeaponSerialization
{
    public static function serialize(weapon:Dynamic):Dynamic
    {
        ...
        if (meleeWeapon != null)
            return MeleeWeaponSerialization.serialize(meleeWeapon);

        var rangedWeapon = Std.downcast(weapon, RangedWeaponPrefab);
        if (rangedWeapon != null)
            return RangedWeaponSerialization.serialize(rangedWeapon);
    ...
    {
        if (data.type == MeleeWeaponSerialization.TYPE)
            return MeleeWeaponSerialization.deserialize(tagRegistry, data);
        if (data.type == RangedWeaponSerialization.TYPE)
            return RangedWeaponSerialization.deserialize(tagRegistry, data);
