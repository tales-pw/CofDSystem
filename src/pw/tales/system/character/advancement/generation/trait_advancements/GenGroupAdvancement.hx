...
using pw.tales.system.utils.Utility;
...
class GenGroupAdvancement<T: ValueTrait, TY:ValueTraitType<T>> extends GenAdvancementItem<T, TY> {
    ...
    public function getGroup(traitType:TY):String throw new AbstractMethod();

    public function getMinValue():Int throw new AbstractMethod();
    ...
    public override function canTraitBeUpdated(trait:T, newValue:Int):Bool {
        if (newValue < this.getMinValue()) return false;
        ...
        if (traitGroupSpent == null) traitGroupSpent = 0;
        ...
        for (item in groupSpentMap.sortedItems()) {
            ...
            var second = 0;
            if (availableFreebuys.length > 0) second = availableFreebuys[1];
        ...
        if (claimedFreebuys.exists(traitGroup)) {
            ...
            if (targetedFreebuy <= biggestFreebuy) targetedFreebuy = biggestFreebuy;
        }
