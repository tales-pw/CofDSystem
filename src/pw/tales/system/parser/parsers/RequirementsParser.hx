...
import pw.tales.system.parser.nodes.NodeGroup;
import pw.tales.system.parser.nodes.NodeOr;
import pw.tales.system.parser.nodes.NodeString;
import pw.tales.system.parser.nodes.NodeTrait;
import pw.tales.system.parser.nodes.NodeTraitRequirement;
...
class RequirementsParser extends Parser {
    ...
    public static final TRAIT_DN:ParseObject<NodeTrait> = ParserHelper.takeWhileNo(
        Parser.whitespace().then(TRAIT_VALUE))
    ...
    public static final STRING_STATEMENT:ParseObject<NodeString> = ParserHelper.inBondaries(Parser.all(), STATEMENT_END)
    .map(function(value) {
        return new NodeString(value);
    }).as("string");

    private static final ARRAY2:Array<ParseObject<Dynamic>> = [STATEMENT, AND_LITERAL, FULL_PARSER];
    public static final AND:ParseObject<NodeAnd> = ARRAY2.seq()
    .map(function(values) {
    ...
    public static var OR:ParseObject<NodeOr> = ARRAY3.seq()
    .map(function(values) {
        var castedResult = cast(values, Array<Dynamic>);
        var node1 = cast(castedResult[0], INodeCheck);
        var node2 = cast(castedResult[2], INodeCheck);
    ...
    public function parse(data:String):INodeCheck {
        ...
        if (!result.status) throw ParseUtil.formatError(result, data);
        return result.value;
    }
}
