...
import pw.tales.system.parser.nodes.INodePoolBuilder;
import pw.tales.system.parser.nodes.NodeInversion;
import pw.tales.system.parser.nodes.NodeNumber;
import pw.tales.system.parser.nodes.NodePoolSum;
...
class ParserPool {
    ...
    public function parse(data:String):INodePoolBuilder {
        ...
        if (!result.status) throw new ParsingException(result.expected.toString());
        return result.value;
    }
}
